...
$marking3 = $_POST['marking3'] ?? null;
$color3 = $_POST['color3'] ?? "#000000";
$success = false;

if ($_SERVER['REQUEST_METHOD'] === 'POST' && $selected_pet_id && $marking1 && $marking2 && $marking3) {
    // Get pet info
    ...
    if ($pet) {
        ...
        // Marking 3
        $marking_path3 = "images/markings/{$pet['type']}/" . basename($marking3);
        if (file_exists($marking_path3)) {
            $mark3 = imagecreatefrompng($marking_path3);
            imagefilter($mark3, IMG_FILTER_COLORIZE,
                hexdec(substr($color3, 1, 2)),
                hexdec(substr($color3, 3, 2)),
                hexdec(substr($color3, 5, 2)));
...
<html>
<head>
    <title>Make A Custom - Three Markings</title>
</head>
<body>
    <h1>Make A Custom - Add Three Markings</h1>
    <?php if ($success): ?>
        <p style="color:green;">Three markings applied! View your pet on your dashboard.</p>
    <?php endif; ?>
    ...
    <?php if ($selected_pet_id): ?>
        <form method="POST">
            ...
            <label for="marking3">Step 4. Choose Third Marking:</label>
            <select name="marking3" id="marking3" required>
                <option value="">-- Select Marking --</option>
                <?php foreach ($markings as $m): ?>
                    <option value="<?= htmlspecialchars($m) ?>" <?= ($marking3==$m)?'selected':'' ?>><?= htmlspecialchars($m) ?></option>
                <?php endforeach; ?>
            </select>
            <label for="color3">Color:</label>
            <input type="color" name="color3" id="color3" value="<?= htmlspecialchars($color3) ?>"><br>
