...
$stmt = $pdo->prepare("SELECT quantity FROM user_items JOIN items ON user_items.item_id = items.id WHERE user_id = ? AND items.name = 'Grand Custom Token'");
$stmt->execute([$user_id]);
$hasToken = $stmt->fetchColumn();

if (!$hasToken) die("You need a Grand Custom Token to use this.");
...
$selected_marking3 = $_POST['marking3'] ?? '';
$selected_color3 = $_POST['color3'] ?? '#000000';
$success = false;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $selected_pet_id && $selected_base) {
    ...
    if ($pet) {
        ...
        // Marking 3
        if ($selected_marking3 && file_exists("images/markings/{$type}/" . $selected_marking3)) {
            $mark3 = imagecreatefrompng("images/markings/{$type}/" . $selected_marking3);
            imagefilter($mark3, IMG_FILTER_COLORIZE,
                hexdec(substr($selected_color3, 1, 2)),
                hexdec(substr($selected_color3, 3, 2)),
                hexdec(substr($selected_color3, 5, 2))
            );
            imagecopy($image, $mark3, 0, 0, 0, 0, imagesx($mark3), imagesy($mark3));
            imagedestroy($mark3);
        }
...
// Consume the token
            $pdo->prepare("UPDATE user_items JOIN items ON user_items.item_id = items.id SET quantity = quantity - 1 WHERE user_id = ? AND items.name = 'Grand Custom Token'")->execute([$user_id]);
...
<html>
<head>
    <title>Custom Maker: 3 Markings + Base</title>
    <style>
...
<h1>Custom Maker: 3 Markings + Base</h1>
<?php if ($success): ?>
...
<?php if ($selected_pet_id): ?>
    <form method="POST" enctype="multipart/form-data" oninput="updatePreview()">
        ...
        <p>
            <strong>Marking 3:</strong>
            <select name="marking3" id="marking3" onchange="updatePreview()">
                <option value="">-- None --</option>
                <?php foreach ($markings as $m): ?>
                    <option value="<?= htmlspecialchars($m) ?>" <?= ($selected_marking3==$m)?'selected':'' ?>><?= htmlspecialchars($m) ?></option>
                <?php endforeach; ?>
            </select>
            <input type="color" name="color3" id="color3" value="<?= htmlspecialchars($selected_color3) ?>">
        </p>
        ...
        function updatePreview() {
            ...
            var m3 = document.getElementById('marking3').value;
                            ...
                            drawTintedMarking(ctx, imgM1, c1, function() {
                                ...
                                if (m2) {
                                    ...
                                    imgM2.onload = function() {
                                        drawTintedMarking(ctx, imgM2, c2, function() {
                                            // Draw marking3
                                            if (m3) {
                                                let imgM3 = new Image();
                                                imgM3.crossOrigin = "";
                                                imgM3.onload = function() {
                                                    drawTintedMarking(ctx, imgM3, c3);
                                                }
                                                imgM3.src = "images/markings/" + type + "/" + m3;
                                            }
                                        });
                                    }
